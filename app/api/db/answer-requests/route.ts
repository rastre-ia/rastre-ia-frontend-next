...
export async function POST(req: NextRequest) {
	...
	try {
		...
		console.log('Answer Request sent: ', answerRequest);

		// Check if the authenticated user is authorized to create this report
		if (answerRequest.policeStationId !== authData.user._id) {
...
export async function GET(req: NextRequest) {
	...
	try {
		...
		// Ensure that the user can only fetch their own data
		...
		let answerRequests: AnswerRequestSchemaInterface[] = [];

		if (authData.user.role === RolesEnum.USER) {
			answerRequests =
				await AnswerRequests.find<AnswerRequestSchemaInterface>()
					.where('usersRequested')
					.equals(userId)
					.exec();
		} else if (authData.user.role === RolesEnum.POLICE_STATION) {
			answerRequests =
				await AnswerRequests.find<AnswerRequestSchemaInterface>()
